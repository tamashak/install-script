...
install_caddy2() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-caddy$") ]]; then
    ...
    while read -r -p "Please select how to set up the certificate? (1/automatic application and renewal of certificates, 2/manually set the certificate path, default: 1/automatic application and renewal of certificates): " ssl_option; do
...
install_custom_cert() {
  while read -r -p "Please enter the path to your .crt file (required): " crt_path; do
    if [[ -z "${crt_path}" ]]; then
      echo_content red "Path cannot be empty."
    else
      if [[ ! -f "${crt_path}" ]]; then
        echo_content red "The specified .crt file does not exist."
      else
  ...
  while read -r -p "Please enter the path to your .key file (required): " key_path; do
    if [[ -z "${key_path}" ]]; then
      echo_content red "Path cannot be empty."
    else
      if [[ ! -f "${key_path}" ]]; then
        echo_content red "The specified .key file does not exist."
      else
...
install_trojan_panel_ui() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-ui$") ]]; then
    ...
    read -r -p "Please enter the port of your Trojan Panel frontend (default: 8888): " trojan_panel_ui_port
...
install_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel$" -f "status=running") ]]; then
      ...
      echo_content yellow "System administrator default username: sysadmin, default password: 123456. Please log in to the management panel and change the password as soon as possible."
...
update_trojan_panel_ui_port() {
  if [[ -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
    ...
    read -r -p "Please enter a new port number for Trojan Panel frontend (default: 8888): " trojan_panel_ui_port
    [[ -z "${trojan_panel_ui_port}" ]] && trojan_panel_ui_port="8888"
...
echo_content green "3. Install Trojan Panel Kernel"
...
echo_content yellow "21. Fault Detection"
echo_content yellow "22. Log Query"
echo_content yellow "23. Version Query"
