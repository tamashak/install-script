...
install_caddy_tls() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-caddy$") ]]; then
    ...
    while read -r -p "Please choose how to set up SSL certificates (1/automatic certificate request and renewal, 2/manual certificate path setup, default: 1/automatic certificate request and renewal): " ssl_option; do
      if [[ -z ${ssl_option} || ${ssl_option} == 1 ]]; then
        while read -r -p "Please choose how to request a certificate (1/acme, 2/zerossl, default: 1/acme): " ssl_module_type; do
          if [[ -z "${ssl_module_type}" || ${ssl_module_type} == 1 ]]; then
            ...
            echo_content red "You cannot input any character other than 1 or 2."
          fi
        done
...
EOF
        ...
        if [[ ${nginx_https} != 0 ]]; then
          echo_content red "You cannot input any character other than 0 or 1."          
        else
          cat >${NGINX_CONFIG} <<-EOF
server {
    listen       ${nginx_port};
...
EOF
  ...
  echo_content yellow "Default system administrator username: sysadmin, default password: 123456. Please log in to the administration panel promptly and change the password."
...
update_trojan_panel_ui_port() {
  if [[ -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
    ...
    trojan_panel_ui_port=$(grep -oP '(?<=listen ).*(?= ssl)' ${UI_NGINX_CONFIG})    
